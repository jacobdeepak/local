...
_global.HTMLCS_WCAG2AAA = {
    ...
    getMsgInfo: function(code) {
        ...
        var retval = [ [ "Success Criterion", successCritStr ], [ "Suggested Techniques", techniquesStr.join(" ") ] ];
        return retval;
    }
};
...
_global.HTMLCS_WCAG2AAA_Sniffs_Principle1_Guideline1_3_1_3_1 = {
    ...
    testSemanticPresentationRole: function(element) {
        if (element.hasAttribute("role") && element.getAttribute("role") === "presentation") {
            ...
            if (children.length) {
                HTMLCS.addMessage(HTMLCS.ERROR, element, 'This element\'s role is "presentation" but contains child elements with semantic meaning.', "F92,ARIA4");
            }
        }
    },
    /**
    ...
    testLabelsOnInputs: function(element, top, muteErrors) {
        ...
        if (title !== null) {
            if (/^\s*$/.test(title) === true && needsLabel === true) {
                HTMLCS.addMessage(HTMLCS.WARNING, element, 'This form control has a "title" attribute that is empty or contains only spaces. It will be ignored for labelling test purposes.', "H65");
            } else {
        ...
        if (element.hasAttribute("aria-label") === true) {
            if (HTMLCS.util.hasValidAriaLabel(element) === false) {
                HTMLCS.addMessage(HTMLCS.WARNING, element, 'This form control has an "aria-label" attribute that is empty or contains only spaces. It will be ignored for labelling test purposes.', "ARIA6");
            } else {
        ...
        if (element.hasAttribute("aria-labelledby") === true) {
            if (HTMLCS.util.hasValidAriaLabel(element) === false) {
                HTMLCS.addMessage(HTMLCS.WARNING, element, 'This form control contains an aria-labelledby attribute, however it includes an ID "' + element.getAttribute("aria-labelledby") + '" that does not exist on an element. The aria-labelledby attribute will be ignored for labelling test purposes.', "ARIA16,ARIA9");
            } else {
        ...
        if (!(muteErrors === true)) {
            if (hasLabel !== false && needsLabel === false) {
                ...
                if (inputType === "hidden") {
                    HTMLCS.addMessage(HTMLCS.WARNING, element, "This hidden form field is labelled in some way. There should be no need to label a hidden form field.", "F68.Hidden");
                } else if (element.getAttribute("hidden") !== null) {
                    HTMLCS.addMessage(HTMLCS.WARNING, element, 'This form field is intended to be hidden (using the "hidden" attribute), but is also labelled in some way. There should be no need to label a hidden form field.', "F68.HiddenAttr");
                }
            } else if (hasLabel === false && needsLabel === true) {
                // Needs label.
                HTMLCS.addMessage(HTMLCS.ERROR, element, "This form field should be labelled in some way." + " " + 'Use the label element (either with a "for" attribute or wrapped around the form field), or "title", "aria-label" or "aria-labelledby" attributes as appropriate.', "F68");
            }
        }
        return hasLabel;
    },
        ...
        if (element === top) {
            ...
            keyboardTriggers.forEach(function(elem) {
                if (HTMLCS.util.isKeyboardNavigable(elem) === false) {
                    HTMLCS.addMessage(HTMLCS.WARNING, elem, "Ensure the functionality provided by an event handler for this element is available through the keyboard", "G90");
                }
            });
            var dblClickEls = HTMLCS.util.getAllElements(top, "*[ondblclick]");
            for (var i = 0; i < dblClickEls.length; i++) {
...
_global.HTMLCS_WCAG2AAA_Sniffs_Principle3_Guideline3_2_3_2_2 = {
    ...
    checkFormSubmitButton: function(form) {
        ...
        //end if
        if (ok === false) {
            HTMLCS.addMessage(HTMLCS.ERROR, form, 'This form does not contain a submit button, which creates issues for those who cannot submit the form using the keyboard. Submit buttons are INPUT elements with type attribute "submit" or "image", or BUTTON elements with type "submit" or omitted/invalid.', "H32.2");
        }
    }
};
        ...
        for (var el = 0, ll = elements.length; el < ll; el++) {
            ...
            if (valueFound === false) {
                ...
                if (requiredValue === "_content") {
                    ...
                    warning = true;
                    msg = "This " + msgNodeType + " does not have an initially selected option." + " " + "Depending on your HTML version, the value exposed to an accessibility API may be undefined.";
                } else if (requiredValue.charAt(0) === "@") {
